...
use axum::{
    body::{Body, Bytes}, extract::{path, State}, http::{Error, HeaderMap, Method, Response, StatusCode}, response::IntoResponse, routing::{any, delete, get, put}, Router
};
...
async fn on_any_handler(Path(path) : Path<String>, method : Method, State(state) : State<Arc<RwLock<InMemoryLoader>>>) -> Result<impl IntoResponse, (StatusCode, String)> {
